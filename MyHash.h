...
void MyHash<KeyType, ValueType>::associate(const KeyType& key, const ValueType& value)
{
	...
	// If there are no duplicate keys, create a new HashNode and add to list
	// Otherwise change duplicate's value
	if (curr == nullptr)
	{
		m_itemNum++;
		curr = new HashNode(key, value);

		// If bucket is empty, add item at the front
		// Otherwise add item to the end of linked list
		if (prev == nullptr)
			htable[hashID] = curr;
		else
			prev->m_next = curr;
